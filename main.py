"""
Top 6 Crypto Auto-Analysis Telegram Bot (GitHub Actions Edition)
Reads secrets from GitHub Secrets natively.
Detects which coin to analyze based on the current UTC time.
"""
...
COLORS = {
    'bg': '#0d1117', 'grid': '#1e2a3a', 'text': '#8b949e',
    'green': '#26a69a', 'red': '#ef5350', 'blue': '#42a5f5',
    'purple': '#ab47bc', 'orange': '#ffa726', 'btc': '#f7931a',
}
...
class CryptoAnalyzer:
    ...
    def calculate_indicators(self, df):
        ...
        if bb_res is not None:
            ...
            df['bb_middle'] = bb_res['BBM_20_2.0']
            df['bb_lower'] = bb_res['BBL_20_2.0']

        stoch_res = ta.stoch(df['high'], df['low'], df['close'], k=14, d=3, smooth_k=3)
        if stoch_res is not None:
            df['stoch_k'] = stoch_res['STOCHk_14_3_3']
            df['stoch_d'] = stoch_res['STOCHd_14_3_3']
    ...
    def _create_chart(self, symbol, df, timeframe):
        ...
        ax1 = fig.add_subplot(gs[0]); ax1.set_facecolor(COLORS['bg']); self._draw_candles(ax1, df_d)
        ...
        ax1.plot(x, df_d['bb_middle'].values, color='#666', lw=0.7, ls='--', alpha=0.5)
        ...
        ax2 = fig.add_subplot(gs[1]); ax2.set_facecolor(COLORS['bg'])
        ...
        ax2.set_ylabel('RSI', color=COLORS['text'], fontsize=9); ax2.set_ylim(0, 100); self._style_axis(ax2, n, df_d)

        ax3 = fig.add_subplot(gs[2]); ax3.set_facecolor(COLORS['bg'])
    ...
    def _style_axis(self, ax, n, df):
        ...
        ax.set_xticks(ticks); ax.set_xticklabels([df.index[i].strftime('%m/%d %H:%M') for i in ticks], fontsize=7)
        ax.tick_params(colors=COLORS['text'], labelsize=8); ax.grid(True, color=COLORS['grid'], ls='--', alpha=0.4)
...
def get_scheduled_task():
    """Determines which coin and timeframe to run based on current UTC time"""
    now = datetime.utcnow()
    h, m = now.hour, now.minute
    
    # 1D Timeframe schedule (09:00 to 10:15)
    if h == 9 and m < 15:   return 'BTC/USDT', '1d'
    if h == 9 and m < 30:   return 'ETH/USDT', '1d'
    if h == 9 and m < 45:   return 'BNB/USDT', '1d'
    if h == 10 and m < 0:   return 'SOL/USDT', '1d'  # 09:45
    if h == 10 and m < 15:  return 'XRP/USDT', '1d'
    if h == 10 and m < 30:  return 'DOGE/USDT', '1d'
    
    # 4H Timeframe schedule (21:00 to 22:15)
    if h == 21 and m < 15:  return 'BTC/USDT', '4h'
    if h == 21 and m < 30:  return 'ETH/USDT', '4h'
    if h == 21 and m < 45:  return 'BNB/USDT', '4h'
    if h == 22 and m < 0:   return 'SOL/USDT', '4h'  # 21:45
    if h == 22 and m < 15:  return 'XRP/USDT', '4h'
    if h == 22 and m < 30:  return 'DOGE/USDT', '4h'
